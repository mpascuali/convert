...
public class ConverteUmParaUm implements Serializable {
	 ...
	 * 
	 * ConversaoParaPDF algoritmo = null; String nomeArquivo =
	 * listaDeArquivos.get(i).getFile().getFileName(); String extensao =
	 * nomeArquivo.substring(nomeArquivo.length() - 3);
	 * 
	 * if (extensao.equalsIgnoreCase("txt")) {
	 * 
	 * algoritmo = new ConversaoAPartirDeTexto();
	 ...
	 * extensao.equalsIgnoreCase("jpeg") || extensao.equalsIgnoreCase("png")) {
	 * 
	 * algoritmo = new ConversaoAPartirDeImagem();
	 * 
	 * }
	 * 
	 * try { byte[] arquivo = listaDeArquivos.get(i).getFile().getContents();
	 * byte[] pdf = algoritmo.converterDocumento(arquivo); String caminhoPdf =
	 * "C:/Users/Marcio Pascuali/Desktop/Arquivos/ConvertidosPdf/" + nomeArquivo
	 * + ".pdf"; listaDeCaminho.add(caminhoPdf); new
	 * Bytes().escreverArquivosEmBytes(pdf, caminhoPdf);
	 * 
	 ...
	 * listaDeArquivos.get(i).getFile().getFileName() +
	 * " foi convertido para PDF");
	 * FacesContext.getCurrentInstance().addMessage(null, message);
	 * 
	 * PretoBranco pb = new PretoBranco(); pb.convertePretoBranco(caminhoPdf);
	 * 
	 * System.out.println(caminhoPdf);
	 * 
	 * } catch (Exception e) { System.out.println(
	 * "OpenOffice desabilitado, contate o Administrator"); FacesMessage message
	 * = new FacesMessage("Falha ao converter " +
	 * listaDeArquivos.get(i).getFile().getFileName(),
	 * "Motivo: OpenOffice desabilitado, por favor contate o Administrator");
	 * FacesContext.getCurrentInstance().addMessage(null, message); } }
	 * 
	 * return listaDeCaminho; }
	 */
