...
function charts() {
	...
	var data = [];
	var graphSize = 10;
	function getOnlineGraphData(period) {
		data = loadOnlineGraph(period,graphSize);
		var res = [];
	...
	{	
		...
		var plot = $.plot($("#serverload"), [ getOnlineGraphData('hour') ], options);
		function update() {
			plot.setData([ getOnlineGraphData('hour') ]);
	...
	if($("#realtimechart").length)
	{
		...
		var plot = $.plot($("#realtimechart"), [getOnlineGraphData('hour')  ], options);
		function update() {
			plot.setData([ getOnlineGraphData('hour') ]);
			// since the axes don't change, we don't need to call plot.setupGrid()
			plot.draw();
